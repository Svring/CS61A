...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.operator.eval(env).apply([oprand.eval(env) for oprand in self.operands])

        # '绕这么大一个圈子，vpn还断了没法看答案，我差点破防了'
        # 'back to here, first version was wrong....cause more errors during the implementation of LambdaFuction'
        # 'this is the wrong first version: '
        # 'PremitiveFunction(self.operator.eval(env).operator).apply([oprand.eval(env) for oprand in self.operands])'
